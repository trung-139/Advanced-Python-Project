...
class StudentUI:
    def __init__(self, student_major):
        self.student_major = student_major
        self.window = Tk()
        self.window.title("Student UI")
        self.window.config(padx=20, pady=20, bg=ALMOND_FROST)
        
        get_btn_img = PhotoImage(file="img/get_btn_img.png")

        class_list = student_major
        self.clicked_class = StringVar()
        self.clicked_class.set(class_list)
        self.class_id_label = Label(self.window, text=f"CLASS ID:", font=FONT_TEXT,
                                    background=ALMOND_FROST, foreground="#fff")
        self.class_id_label.grid(column=0, row=0, padx=10, pady=10)
        #self.class_id_dropdown = OptionMenu(self.window, self.clicked_class, *class_list)
        #self.class_id_dropdown.grid(column=1, row=0, padx=10, pady=10)
        
        self.get_class_btn = Button(self.window, image=get_btn_img,
        ...
        self.get_class_label = Label(self.window, text="", font=FONT_TEXT,
                                     background=ALMOND_FROST, foreground="#fff")
        self.get_class_label.grid(column=3, row=0, padx=10, pady=10)

        self.clicked_date = StringVar()
        self.date_label = Label(self.window, text="DATE:", font=FONT_TEXT,
                                background=ALMOND_FROST, foreground="#fff")
        self.date_dropdown = ""
        self.get_date_btn = Button(self.window, image=get_btn_img,
                                   highlightthickness=0, borderwidth=0,
                                   activebackground=ALMOND_FROST, command=self.date_label_get)
        self.get_date_label = Label(self.window, text="", font=FONT_TEXT,
                                    background=ALMOND_FROST, foreground="#fff")
        self.get_date_btn["bg"] = ALMOND_FROST

        search_btn_img = PhotoImage(file="img/serach_btn_img.png")
        self.search_btn = Button(self.window, image=search_btn_img, highlightthickness=0, borderwidth=0,
        ...
        self.timetable = Label(self.window, text="", font=FONT_TEXT,
...
# choose 
    ...
    def class_id_label_get(self):
        ...
        self.date_dropdown = OptionMenu(self.window, self.clicked_date, *date_list)
